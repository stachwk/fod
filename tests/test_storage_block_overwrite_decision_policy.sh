...
grep -Fq 'FOD_STORAGE_DECISION_WARMUP_RUNS", 1' "${RUNNER}"
grep -Fq 'FOD_STORAGE_DECISION_RUNS", 3' "${RUNNER}"
grep -Fq 'FOD_STORAGE_DECISION_MAX_ATTEMPTS", 8' "${RUNNER}"
grep -Fq 'FOD_STORAGE_DECISION_FILE_SIZE", "256M"' "${RUNNER}"
grep -Fq 'FOD_STORAGE_DECISION_IO_SIZE", "256M"' "${RUNNER}"
grep -Fq 'FOD_STORAGE_DECISION_HOST_DIRTY_LIMIT_KB", 8192' "${RUNNER}"
grep -Fq 'FOD_STORAGE_DECISION_HOST_COOLDOWN_SECONDS", 20.0' "${RUNNER}"
grep -Fq 'FOD_STORAGE_DECISION_FSYNC_PROBE_COUNT", 8' "${RUNNER}"
grep -Fq 'FOD_STORAGE_DECISION_FSYNC_MEDIAN_LIMIT_MS", 20.0' "${RUNNER}"
grep -Fq 'FOD_STORAGE_DECISION_FSYNC_MAX_LIMIT_MS", 100.0' "${RUNNER}"
grep -Fq 'subprocess.run(["sync"], check=True)' "${RUNNER}"
grep -Fq 'os.fsync(fd)' "${RUNNER}"
grep -Fq 'row["run_quality"] = "warmup"' "${RUNNER}"
grep -Fq 'row["run_quality"] == "clean"' "${RUNNER}"
