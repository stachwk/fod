...
#
# Run the isolated primary/replica benchmark for a matrix of block sizes.
...
for block_size in ${BLOCK_SIZES}; do
    ...
    log="${ARTIFACT_DIR}/${safe_block_size}.log"

    echo "=== MATRIX block_size=${block_size} file_size=${FILE_SIZE} ==="
    set +e
    FIO_FILE_SIZE="${FILE_SIZE}" \
    FIO_BLOCK_SIZE="${block_size}" \
    REPLICA_READ_LABEL="${LABEL}-${safe_block_size}" \
    bash "${SINGLE}" 2>&1 | tee "${log}"
    status=${PIPESTATUS[0]}
    set -e
    if [[ "${status}" -ne 0 ]]; then
        echo "Benchmark failed for block_size=${block_size}; log=${log}" >&2
        exit "${status}"
    fi

    result="$(grep '^PERF_RESULT ' "${log}" | tail -n 1)"
    [[ -n "${result}" ]] || {
        echo "Missing PERF_RESULT for block_size=${block_size}" >&2
        exit 1
    }
