...
fod_test_init_schema() {
  ...
  status_output="$(
    POSTGRES_DB="${POSTGRES_DB}" POSTGRES_USER="${POSTGRES_USER}" POSTGRES_PASSWORD="${POSTGRES_PASSWORD}" \
      FOD_CONFIG="${FOD_CONFIG}" "${FOD_MKFS_BIN}" status 2>/dev/null || true
  )"
  if grep -Fq "FOD ready: yes" <<<"${status_output}"; then
    return 0
  fi
  POSTGRES_DB="${POSTGRES_DB}" POSTGRES_USER="${POSTGRES_USER}" POSTGRES_PASSWORD="${POSTGRES_PASSWORD}" FOD_CONFIG="${FOD_CONFIG}" "${FOD_MKFS_BIN}" init --schema-admin-password "${FOD_SCHEMA_ADMIN_PASSWORD}"
}
...
fod_test_start_mount() {
  ...
  POSTGRES_DB="${POSTGRES_DB}" POSTGRES_USER="${POSTGRES_USER}" POSTGRES_PASSWORD="${POSTGRES_PASSWORD}" FOD_CONFIG="${FOD_CONFIG}" FOD_BOOTSTRAP_BIN="${FOD_BOOTSTRAP_BIN}" FOD_USE_RUST_FUSE=1 FOD_USE_FUSE_CONTEXT=1 FOD_ALLOW_OTHER="${FOD_ALLOW_OTHER}" FOD_SELINUX_CONTEXT="${FOD_SELINUX_CONTEXT}" FOD_SELINUX_FSCONTEXT="${FOD_SELINUX_FSCONTEXT}" FOD_SELINUX_DEFCONTEXT="${FOD_SELINUX_DEFCONTEXT}" FOD_SELINUX_ROOTCONTEXT="${FOD_SELINUX_ROOTCONTEXT}" "${start_cmd[@]}" >"${LOG_FILE}" 2>&1 &
  FOD_PID=$!
