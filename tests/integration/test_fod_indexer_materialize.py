...
def apply_database_env(dsn: dict[str, str]) -> None:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="fod-indexer-materialize-") as mount_dir:
            with FODMount(str(ROOT)) as mount:
                ...
                scan_output = run_indexer(["scan", "--source", "smoke"])
                ...
                hash_output = run_indexer(["hash", "--source", "smoke", "--candidates-only"])
                ...
                report_output = run_indexer(["report", "duplicates"])
                ...
                dry_run_output = run_indexer(["plan-import", "--dry-run"])
                ...
                materialize_output = run_indexer(["materialize", "--source", "smoke"])
