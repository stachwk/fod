...
- [ ] Dodać pełniejszy replay in-flight SQL po błędach.
  - Progress: `DbRepo::query_rows_text()` now retries read-only SQL once after a transient disconnect, and `DbRepo::exec()` now retries the idempotent replayable command set once too; `acquire_flock_lease()`'s `lock_leases` upsert now carries a durable `request_token`, `register_client_session()` now does too, `touch_data_object()`'s pure metadata touch, `adopt_source_data_object()`'s source/destination state confirmation, and the row reassignment updates inside `purge_primary_file()` for `data_blocks`, `data_extents`, and `copy_block_crc` are also replay-safe now. `purge_primary_file()` itself now retries once after a lost COMMIT because the missing file row confirms the terminal state. The create-entry family now uses `transactional_replayable()` too, so `create_hardlink()`, `create_symlink()`, `create_directory()`, `create_file()`, and `create_special_file()` can retry a lost COMMIT and then confirm the already-committed natural-key row. `create_data_object()` is now request-token-backed too, so a replayed COMMIT returns the already-chosen `id_data_object` without double-incrementing `reference_count`. `promote_hardlink_to_primary()` now also keeps a durable request-token row, so the remaining hardlink-promotion path can replay once without choosing a different survivor on reconnect, and `touch_client_session_owner_key()` now has commit-disconnect smoke coverage too. `persist_file_blocks_with_crc_flag()` and `persist_file_extents_with_crc_flag()` now also replay safely on commit disconnect, and the same smoke suite covers the block, extent, copy-CRC, set-file-size, lock, promotion, owner-key-touch, and session-registration replay paths. `index_import_plan_entries` inserts are replay-safe via `DELETE + INSERT`, and `index_scan_runs` plus `index_import_plans` now carry explicit `request_token` columns so retry can return the same running row, but broader transactional replay still needs a separate design because commit outcome after a disconnect remains ambiguous.
  ...
  - Progress: the first wrapper pass now uses `transactional_replayable()` for `set_file_size()`, `persist_lock_range_state_blob()`, and `replace_lock_range_state_blob_for_owner()`, and `rust_hotpath/tests/transactional_replay_smoke.rs` covers the commit-disconnect replay path for all three.
  - Progress: the replay envelope is now defined as request-token-backed transactions with a durable post-reconnect state probe; ambiguous commit outcomes stay fail-closed unless the row-level marker proves the transaction already committed.
  - Progress: the smoke suite now covers a multi-statement `create_file()` transaction too, with the proxy dropping after the file insert has been parsed and executed so the retry path is exercised past the first statement.
  - Progress: the create-entry family now confirms a replayed `SQLSTATE 23505` against the existing natural key, so `create_hardlink()`, `create_symlink()`, `create_directory()`, `create_file()`, and `create_special_file()` can return the already-committed row after a lost acknowledgement instead of failing closed on the duplicate insert.
