...
- [ ] Review `rust_indexer` allocation and buffer reuse after heap or allocation profiling.
  - Follow-up note: `profile-indexer-alloc` now captures `fod-indexer` runs through `heaptrack`, `valgrind massif`, or `/usr/bin/time -v`; run it on representative `scan` and `hash` workloads before changing runtime buffers.
