...
- [ ] Measure repeated full-overwrite data-object swap runs for WAL, relation growth, and dead-tuple cleanup behavior. The immediate swap path removes non-HOT updates, but still creates insert/delete churn; if this becomes the next bottleneck, evaluate delayed cleanup or object-GC policy without reintroducing changed-payload conflict updates.
