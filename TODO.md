...
- [ ] Use `profile-pg-data-blocks-merge-explain`, `profile-pg-data-blocks-bloat`, and `profile-pg-wal-delta` evidence before any runtime `data_blocks` merge optimization. Current diagnostics show the live large-copy path still spending materially in server-side `COPY` and the two real `data_blocks` merges, with low `n_dead_tup` and stable WAL around `12.85-13.35 MB` per 64 MiB workload. Batch-size sensitivity was checked locally and on QNAP; it mainly changed client-side send fragmentation, while WAL stayed in the same band. Next analysis should focus on server-side staging/COPY cost, `idx_data_blocks_object_order` conflict lookup/update cost, real table/index bloat, WAL impact, and repeated QNAP/local variance before touching production SQL.
