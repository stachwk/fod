...
- [ ] Deliver the `Storage Engine v2` project described in `docs/storage-engine-v2-plan.md`, preserving 4 KiB logical blocks and the default block path while making large sequential physical persistence bounded and opt-in.
  ...
  - Phase B: add a sequential segment builder and direct segment persistence only after Phase A proves useful.
    ...
    - Remaining constraint: the repeated 64 MiB large-copy run is still slower on extents (`14.07 MiB/s`) than blocks (`18.50 MiB/s`). Profiling points to repeated extent reads during copy rather than segment payload assembly. Keep extents opt-in and do not route large-copy broadly through `NewObjectSequential` until range-oriented extent reads or direct object adoption remove that read amplification.
  - Phase C: classify persistence semantics and add replay-confirmed, append-only persistence for new sequential objects.
    ...
    - Measured gate: the repeated 64 MiB large-file run averaged `94.55 MiB/s` on append-only extents versus `46.39 MiB/s` on blocks, with 64 physical extent rows and about `1.03 MB` mean WAL. Large-copy remained slower on extents (`12.14 MiB/s`) than blocks (`16.07 MiB/s`) because the source read path repeatedly fetches extent ranges. Extents therefore remain opt-in.
  - Phase D: decide from measured results whether an object segment manifest and chunk store are still needed; first separate the remaining extent-read amplification from problems that actually require a manifest.
