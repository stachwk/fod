...
- [ ] Use `profile-pg-data-blocks-merge-explain` evidence before any runtime `data_blocks` merge optimization. The first temp-table reproducer showed fresh insert around `236 ms`, identical conflict update around `308 ms`, changed conflict update around `359 ms`, while the live large-copy run still spent about `1219 ms` in server-side `COPY` and about `936 ms` in the two real `data_blocks` merges. Reassess staging indexes, table/index bloat, WAL impact, and batch-size sensitivity before touching production SQL.
