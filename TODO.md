...
- [ ] Extend the PostgreSQL WAL-pressure benchmark into a larger or checkpoint-forcing variant if we need direct checkpoint tuning signal; the current short run shows WAL volume and fsync pressure, but not checkpoint counters, on the QNAP backend.
