...
- [ ] Implement a transactional PostgreSQL payload quota for `max_fs_size_bytes`.
  - The current setting is only a reported `statfs` capacity ceiling. A real quota must serialize the post-write payload total inside the same PostgreSQL transaction across all mounts and roll back with `ENOSPC` when the limit would be exceeded. Do not add a process-local preflight check because concurrent mounts could pass it simultaneously.
