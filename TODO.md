...
- [ ] Design and implement role-aware multi-endpoint PostgreSQL routing for one FOD cluster, with several writable entrypoints and several read-only replicas, without encoding endpoint roles through list order.
  ...
  - Implementation note (2026-07-23): phase 4 stage 2 has a first observability foundation. Shared `DbRepo` pool state now records live/idle/active connections, current and peak acquisition waiters, checkout wait, connection creation latency/failures, repository-operation latency/failures, and replay count. The opt-in lane mount logs cumulative per-lane snapshots and process RSS after startup and mount completion. Logical operation queues, in-flight payload bytes, throughput, transaction-specific latency, heartbeat delay, periodic/peak process RSS, and PostgreSQL-side memory pressure remain open before tuning.
