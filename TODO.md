...
- [ ] Revisit `tests/integration/test_fio_sequential_io.sh` extent marker handling after the 2026-07-05 perf-wrapped FUSE profile printed `missing 'FOD extent PoC execution file_id='` while the overall workload still exited with status `0`; reanalyze whether the smoke should fail, skip, or document the accepted missing-marker case.
