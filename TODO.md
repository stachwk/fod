...
- [x] Add a dedicated concurrent two-mount quota regression that places both writes near `max_fs_size_bytes`, proves that only one transaction commits, and verifies `ENOSPC` plus unchanged payload totals for the rejected writer.
  - Revisit this when quota integration tests are next extended; the 2026-07-18 single-mount smoke already verifies `ENOSPC` and complete payload rollback, but it does not force advisory-lock contention between independent processes.
  - Implementation note (2026-07-18): copy paths now reserve rounded target capacity in PostgreSQL before loading or mutating the destination. Two independent repository connections verify that an active reservation prevents a second reservation from consuming the same free capacity, and the mounted `copy_file_range` regression verifies pre-write `ENOSPC`, unchanged destination data, and no leaked reservation. The remaining open scope is forced advisory-lock contention through two mounted processes.
...
- [ ] Design and implement role-aware multi-endpoint PostgreSQL routing for one FOD cluster, with several writable entrypoints and several read-only replicas, without encoding endpoint roles through list order.
  ...
  - Implementation note (2026-07-23): phase 4 stage 2 records pool and payload-flow observability. Shared `DbRepo` pool state records live/idle/active connections, current and peak acquisition waiters, checkout wait, connection creation latency/failures, repository-operation latency/failures, and replay count. Persistence scopes add current/peak logical payload bytes plus input rows, input bytes, failures, and elapsed time per lane and globally across all lane repositories. Logical operation queues, actual batch/file throughput, transaction-specific latency, heartbeat delay, periodic/peak process RSS, and PostgreSQL-side memory pressure remain open before tuning.
...
- [ ] Deliver the Compatibility and FUSE Modernization project without reopening the completed Storage Engine v2 implementation.
  ...
  - [ ] Keep every future production capability experiment isolated to one capability per commit with an explicit semantic contract, correctness gates, and a workload-specific benchmark.
