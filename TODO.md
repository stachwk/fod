...
- [ ] Design and implement role-aware multi-endpoint PostgreSQL routing for one FOD cluster, with several writable entrypoints and several read-only replicas, without encoding endpoint roles through list order.
  ...
  - Implementation note (2026-07-28): `LogicalTaskQueueObservability` now aggregates admitted, queued, active, completed, failed, backpressure, fairness, active-transaction, payload-in-flight, batch, throughput, and accounting-error counters in `fod-rust-monitor`. It is still not wired into FUSE execution.
