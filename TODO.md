...
- [ ] Use `profile-pg-data-blocks-merge-explain`, `profile-pg-data-blocks-bloat`, `profile-pg-wal-delta`, and `profile-pg-table-dml-delta` evidence before any runtime `data_blocks` merge optimization. Current diagnostics show the live large-copy path still spending materially in server-side `COPY` and the two real `data_blocks` merges, with low `n_dead_tup` and stable WAL around `12.85-13.35 MB` per 64 MiB workload. The 2026-07-03 local DML delta on commit `c5d7f24` showed `32768` inserts, `0` updates, `0` HOT updates, and `0` dead-tuple growth for the 64 MiB large-copy workload, so that workload does not answer heap rewrite/HOT eligibility. Next analysis should add a targeted overwrite/conflict workload that forces `ON CONFLICT DO UPDATE`, then compare HOT/non-HOT update deltas before touching production SQL.
