...
- [ ] Remove or regenerate the stale per-crate `rust_*/Cargo.lock` files after confirming no external standalone-crate workflow consumes them. Workspace builds use the root `Cargo.lock`; the nested files still describe old FOD and dependency versions and can mislead dependency inventory.
- [ ] Deliver the Compatibility and FUSE Modernization project without reopening the completed Storage Engine v2 implementation.
  ...
  - [ ] Run useful post-7.31 capability probes one at a time with an explicit semantic contract and benchmark; start with mounted `SYNCFS`/`TMPFILE`/`STATX` fallback behavior, without enabling a production flag.
