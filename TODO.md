...
- [ ] Investigate and document the mounted FOD space-accounting discrepancy where `df` reports `638M` total and `23M` used, `du -hx .` reports `45M`, while the logical sizes shown by `ls -al *` are much larger.
...
- [ ] Add a dedicated concurrent two-mount quota regression that places both writes near `max_fs_size_bytes`, proves that only one transaction commits, and verifies `ENOSPC` plus unchanged payload totals for the rejected writer.
  ...
  - Follow-up: when quota integration tests are explicitly extended, add a forced-expiry regression that verifies both successful renewal with available capacity and `ENOSPC` without payload mutation after another operation claims the released capacity.
