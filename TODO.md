...
- [ ] Use `profile-pg-data-blocks-merge-explain`, `profile-pg-data-blocks-bloat`, and `profile-pg-wal-delta` evidence before any runtime `data_blocks` merge optimization. Current local diagnostics show the live large-copy path still spending about `1121-1219 ms` in server-side `COPY` and about `894-936 ms` in the two real `data_blocks` merges, `data_blocks` at `143 MB` heap / `206 MB` total with low `n_dead_tup`, and a clean single-run WAL delta around `12.86 MB`. Batch-size sensitivity was checked with `FOD_PERSIST_COPY_SEND_BUFFER_BYTES=65536`, default `1 MiB`, and `4194304`; it mainly changed client-side send fragmentation, while elapsed time and WAL stayed in the same band. Reassess staging indexes, real table/index bloat, and WAL impact before touching production SQL.
