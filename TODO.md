...
- [ ] Preserve optional `FOD_PROFILE_IO` summaries from Rust FUSE benchmark tests when profiling is enabled, because the current test support can remove the temporary mount log before the aggregate is visible; revisit this when adding the next profiling harness improvement.
