...
- [ ] Deliver the `Storage Engine v2` project described in `docs/storage-engine-v2-plan.md`, preserving 4 KiB logical blocks and the default block path while making large sequential physical persistence bounded and opt-in.
  ...
  - Phase C: classify persistence semantics and add replay-confirmed, append-only persistence for new sequential objects.
    - Classification completed: `PersistWriteClass` now separates `NewObjectSequential`, `ExistingObjectPatch`, and `TruncateOnly` in the shared hot-path planner. The block/extent execution plan and direct segment path emit the selected class, but still use the unchanged Phase B repository transactions.
    - Next implementation: dispatch `NewObjectSequential` to one replay-confirmed append-only data-object transaction. Keep `ExistingObjectPatch` on the current staging/merge path and `TruncateOnly` on its existing metadata/storage boundary.
