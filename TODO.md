...
- [ ] Add a deterministic local test-database restore target for the state left by `cargo test -p fod-rust-mkfs`.
  - Revisit when the schema test harness is next changed: the migration tests can leave an initialized schema with an empty `fod.config`, while `make init` correctly skips an existing schema. The restore path should remain explicitly test-only and must not reset an operator database by accident.
...
- [ ] Implement and verify an explicit fuser `fsync` callback for per-file durability.
  - Revisit as a separate FUSE durability task: the 2026-07-23 mounted payload-observability probe persisted data through the normal write/flush path, but `sync_all()` also produced fuser's `[Not Implemented] fsync` warning. Define whether `datasync` differs, propagate injected PostgreSQL persistence failures, and prove remount durability instead of relying on the kernel's handling of an unimplemented callback.
