...
POSTGRES_BENCHMARK_LOCAL_PLANNER_PRESET_SHARED_BUFFERS ?= 512MB
POSTGRES_BENCHMARK_LOCAL_PLANNER_PRESET_RANDOM_PAGE_COST ?= 1.1
POSTGRES_BENCHMARK_LOCAL_PLANNER_PRESET_EFFECTIVE_CACHE_SIZE ?= 4GB
POSTGRES_BENCHMARK_LOCAL_PLANNER_PRESET_MAINTENANCE_WORK_MEM ?= 512MB
POSTGRES_BENCHMARK_LOCAL_PLANNER_PRESET_AUTOVACUUM_MAX_WORKERS ?= 3
POSTGRES_BENCHMARK_LOCAL_PLANNER_PRESET_AUTOVACUUM_WORK_MEM ?= 256MB
...
help:
	...
	'  make install-rust-hotpath - build and install the Rust hot-path shared library (respects FOD_CARGO_PROFILE)' \
		...
		'  make postgres-benchmarks-local-planner-preset - run the planner/autovacuum benchmark preset on local Docker only' \
...
postgres-benchmarks-local-planner-preset:
	@$(MAKE) --no-print-directory \
		POSTGRES_SHARED_BUFFERS=$(POSTGRES_BENCHMARK_LOCAL_PLANNER_PRESET_SHARED_BUFFERS) \
		POSTGRES_RANDOM_PAGE_COST=$(POSTGRES_BENCHMARK_LOCAL_PLANNER_PRESET_RANDOM_PAGE_COST) \
		POSTGRES_EFFECTIVE_CACHE_SIZE=$(POSTGRES_BENCHMARK_LOCAL_PLANNER_PRESET_EFFECTIVE_CACHE_SIZE) \
		POSTGRES_MAINTENANCE_WORK_MEM=$(POSTGRES_BENCHMARK_LOCAL_PLANNER_PRESET_MAINTENANCE_WORK_MEM) \
		POSTGRES_AUTOVACUUM_MAX_WORKERS=$(POSTGRES_BENCHMARK_LOCAL_PLANNER_PRESET_AUTOVACUUM_MAX_WORKERS) \
		POSTGRES_AUTOVACUUM_WORK_MEM=$(POSTGRES_BENCHMARK_LOCAL_PLANNER_PRESET_AUTOVACUUM_WORK_MEM) \
		postgres-benchmarks-local
