...
fn postgres_envs() -> Vec<(&'static str, String)> {
    ...
    let password = selected_env_value("FOD_PG_PASSWORD", "POSTGRES_PASSWORD", "cichosza");
    vec![
        ("FOD_PG_HOST", host.clone()),
        ("FOD_PG_PORT", port.clone()),
        ("FOD_PG_DBNAME", dbname.clone()),
        ("FOD_PG_USER", user.clone()),
        ("FOD_PG_PASSWORD", password.clone()),
        ("POSTGRES_HOST", host),
        ("POSTGRES_PORT", port),
        ("POSTGRES_DB", dbname),
        ("POSTGRES_USER", user),
...
fn schema_21_upgrade_applies_monitor_stats_and_status_checks_latest_shape() {
    ...
    conn.exec(
        "DROP TABLE fod.monitor_session_stats;
         CREATE INDEX idx_data_blocks_data_object_id
             ON fod.data_blocks (data_object_id);
         UPDATE fod.schema_version SET version = 21, applied_at = NOW();",
    )
    .expect("prepare schema version 21 with legacy redundant data_blocks index");
    assert!(
