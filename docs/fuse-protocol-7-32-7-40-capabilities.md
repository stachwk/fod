...
| `FOPEN_NOFLUSH` | 7.35 | Available through negotiated protocol | Exposed as `FopenFlags::FOPEN_NOFLUSH` | Could remove duplicate close-time flush callbacks | High: FOD `flush` and `fsync` return persistence errors, while `release` can only send a FUSE error reply that is not guaranteed to reach `close()` | Clean close, dirty close, duplicated descriptors, lock-owner cleanup, injected DB error, remount, callback benchmark | Do not enable until final persistence and error propagation no longer depend on `flush` or a caller-visible `fsync` |
