...
- The project has a working PostgreSQL-backed FUSE core with integration tests, documented runtime profiles, and a shared Rust indexing core.
...
- The FUSE runtime is now Rust-backed: mount entrypoints, namespace mutations, lookup helpers, the `getattr()` / `readdir()` query layer, and `rust_indexer` all live in Rust rather than in Python.
...
- The shared `fod-indexer` core now owns source registration, scanning, hashing, duplicate detection, import planning, materialization, and cleanup through one capability model instead of a separate crawler per source kind.
...
- The long-term architecture direction is now explicit: Rust owns the runtime and hot-path engine, while non-runtime docs and test harnesses stay separate from the runtime codebase.
...
- shared Rust `fod-indexer` core with capability-driven source kinds and source-scoped planning/materialization
- bounded transactional replay helper with durable outcome confirmation probes
...
- keep the `THROUGHPUT_SYNC=1` comparison updated, since it is the main durability-vs-throughput reference and still remains workload-sensitive rather than globally faster
- keep the current `bulk_write` vs `metadata_heavy` large-copy comparison as a baseline and continue tuning the Rust write/copy hot path only where measurements justify it; the Python runtime fallback is gone
- keep the shared `fod-indexer` source capability model, source naming heuristics, and adapter boundary current as new source kinds are added
...
- keep the transactional replay envelope frozen in `docs/transactional-replay-project.md`; wider in-flight SQL replay remains a separate project
