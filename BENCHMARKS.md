...
- The current `bulk_write` vs `metadata_heavy` large-copy comparison is the baseline for profile selection; the Rust hot path lives in `rust_hotpath/` and is linked into `fod-rust-fuse` and `fod-rust-indexer` through the workspace `rlib`, covering the copy planner, changed-run packing, persist padding, read assembly, logical resize planning for `truncate()`/`fallocate()`, startup config queries, and the first repository lookups/mutations while the historical Python-era baseline remains available for reference.
