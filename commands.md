...
Base commit at execution time: `55ff34f`

- `source ~/.venv/bin/activate && mempalace search "FOD fod-monitor pg_lanes monitoring process PostgreSQL pool metrics" --wing myai`
- `git status --short --branch && cat fod_version.txt && nl -ba Cargo.toml | sed -n '1,50p'`
- inspection of `rust_fuse/src/pg_lanes.rs`, `rust_hotpath/src/pg.rs`, `ROADMAP.md`, `TODO.md`, and `docs/postgresql-multi-endpoint-phase-4.md`
- `rg -n 'Observ|Metric|metrics|Memory|rss|pg_stat|Pool|Lane|snapshot|diagnostic|Monitor' rust_fuse/src/pg_lanes.rs rust_hotpath/src/pg.rs rust_fuse/src`
- `cargo fmt --all`
- `RUSTFLAGS="-D warnings" cargo check --workspace --locked` (passed for FOD `3.2.38`)
- `cargo fmt --all -- --check` (passed)
...
Base commit at execution time: `1b69421`

- `source ~/.venv/bin/activate && mempalace search "FOD Stage 3 queue task classification monitor logical task metrics" --wing myai`
- `git status --short --branch && cat fod_version.txt && rg -n 'queue|queued|task|Stage 3|logical task|active transaction|throughput' rust_monitor rust_fuse/src rust_hotpath/src docs/postgresql-multi-endpoint-phase-4.md TODO.md ROADMAP.md`
- added neutral Stage 3 queue and throughput contracts to `rust_monitor/src/lib.rs`
- `cargo fmt --all -- --check` (passed)
...
- `make test-version` (`7` passed)
- `make test-df` (passed)
- `FOD_STATFS_CACHE_TTL_SECONDS=0 .venv/bin/python - <<'PY' ... PY` (mounted live-limit probe passed; decreasing the PostgreSQL limit by 4096 bytes changed `f_blocks` from `2621440` to `2621439` without remounting)
- `cargo test -p fod-rust-hotpath --lib` (`80` passed)
- `cargo test -p fod-rust-fuse --bin fod-rust-fuse` (`27` passed)
- `FOD_PROFILE_IO=1 make test-fio-sequential-io` (block and opt-in extent cases passed)
- `make test-fio-sequential-io-strace` (block and opt-in extent cases passed)
...
git status --short --branch
cat fod_version.txt
git add README.md README.pl commands.md conclusions.md docs/FOD_3_3_22_ROCKY_SELINUX.md
git commit -m "FOD 3.3.22: clarify Rocky SELinux scope"
git diff --check HEAD~1..HEAD
git diff HEAD~1..HEAD --stat
git show --oneline --decorate --no-renames --stat HEAD
git status --short --branch
