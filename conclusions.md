...
- FOD 3.2.81 removes two compiler warnings from `choose_persist_execution_plan_preserves_plan_shape` without changing persist behavior. `PersistPayloadPlan` currently has only the `Blocks` variant, so the previous wildcard arms were unreachable; the test now uses irrefutable `let` destructuring and keeps the same assertions for existing-object patch and truncate-only plans.
- FOD 3.2.82 adds a process-local in-flight writable-primary generation fence. A primary generation transition now marks the transition pending and waits for all operations using the guarded writable-primary target on that generation to drain; this conservatively includes primary reads as well as write/control work. An acquisition that becomes stale before its operation starts is recycled/discarded and retried instead of running against the old generation. Replayable failures explicitly release their own operation guard before requesting failover, preventing self-deadlock. This is not cross-process STONITH or an external lease epoch, which remains a separate requirement.
