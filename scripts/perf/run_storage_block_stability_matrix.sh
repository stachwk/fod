...
#
# Repeated stability benchmark for storage block-size candidates. Each run uses
# the existing isolated random-payload matrix, therefore it gets a fresh
# PostgreSQL primary/replica lab and its own PostgreSQL profiler. The default
# order is rotated between rounds to reduce time/temperature/order bias:
#   round 1: 16K, 32K, 64K
#   round 2: 32K, 64K, 16K
#   round 3: 64K, 16K, 32K
...
for cmd in bash git awk sort head tail tee; do
...
case "${REPEATS}" in
    ''|*[!0-9]*)
        echo "FOD_STORAGE_STABILITY_REPEATS must be a positive integer" >&2
        exit 2
        ;;
esac
if (( REPEATS < 1 )); then
    echo "FOD_STORAGE_STABILITY_REPEATS must be >= 1" >&2
    exit 2
fi
...
median_for() {
    local size="$1"
    local column="$2"
    awk -F '\t' -v size="${size}" -v column="${column}" 'NR > 1 && $3 == size {print $column}' "${RUNS}" \
        | sort -n \
        | awk '
            {values[NR]=$1}
            END {
...
min_for() {
    ...
    awk -F '\t' -v size="${size}" -v column="${column}" '
        NR > 1 && $3 == size {if (!seen || $column < min) min=$column; seen=1}
...
max_for() {
    ...
    local column="$2"
    awk -F '\t' -v size="${size}" -v column="${column}" '
        NR > 1 && $3 == size {if (!seen || $column > max) max=$column; seen=1}
        END {if (seen) printf "%.3f\n", max; else print "0"}
    ' "${RUNS}"
}
...
count_for() {
    local size="$1"
    awk -F '\t' -v size="${size}" 'NR > 1 && $3 == size {count++} END {print count + 0}' "${RUNS}"
}
...
printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "storage_block_size" "runs" \
    "median_primary_write_mib_s" "min_primary_write_mib_s" "max_primary_write_mib_s" "write_spread_pct" \
    "median_primary_read_mib_s" "median_replica_read_mib_s" \
    "median_copy_mean_ms" "median_insert_mean_ms" "median_sql_flush_mean_ms" \
    "median_wal_bytes_delta" "median_wal_sync_time_delta_ms" "max_wal_sync_time_delta_ms" \
    "median_wal_buffers_full_delta" >"${MEDIANS}"

for storage_block_size in "${BLOCK_SIZES[@]}"; do
    runs="$(count_for "${storage_block_size}")"
    median_write="$(median_for "${storage_block_size}" 4)"
    min_write="$(min_for "${storage_block_size}" 4)"
    max_write="$(max_for "${storage_block_size}" 4)"
    spread_pct="$(awk -v min="${min_write}" -v max="${max_write}" -v median="${median_write}" 'BEGIN {
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "${storage_block_size}" "${runs}" \
        "${median_write}" "${min_write}" "${max_write}" "${spread_pct}" \
        "$(median_for "${storage_block_size}" 5)" \
        "$(median_for "${storage_block_size}" 6)" \
        "$(median_for "${storage_block_size}" 10)" \
        "$(median_for "${storage_block_size}" 14)" \
        "$(median_for "${storage_block_size}" 15)" \
        "$(median_for "${storage_block_size}" 17)" \
        "$(median_for "${storage_block_size}" 22)" \
        "$(max_for "${storage_block_size}" 22)" \
        "$(median_for "${storage_block_size}" 18)" >>"${MEDIANS}"
done

best_size="$(awk -F '\t' 'NR > 1 {if (!seen || $3 > best) {best=$3; size=$1; seen=1}} END {print size}' "${MEDIANS}")"
best_write="$(awk -F '\t' -v size="${best_size}" '$1 == size {print $3}' "${MEDIANS}")"
...
cat "${MEDIANS}"
echo "best_median_primary_write_block_size=${best_size}"
echo "best_median_primary_write_mib_s=${best_write}"
