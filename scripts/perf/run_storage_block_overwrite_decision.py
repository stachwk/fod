...
Workloads:
  * 4 KiB random overwrite of an existing incompressible 1 GiB file
  * 16 KiB random overwrite
  * 64 KiB random overwrite
  * 512 KiB sequential write

Every successful cell is measured three times by default on a fresh Docker
primary/replica database. Infrastructure failures are retried, but successful
slow runs are retained: a real read-modify-write penalty must not be filtered
out as a "stall". Stability is judged from the spread of successful runs.
"""
...
def git_short() -> str:
    proc = subprocess.run(["git", "rev-parse", "--short", "HEAD"], cwd=ROOT, text=True,
...
def load_config() -> Config:
    ...
    return Config(
        ...
        file_size=os.environ.get("FOD_STORAGE_DECISION_FILE_SIZE", "1G"),
        io_size=os.environ.get("FOD_STORAGE_DECISION_IO_SIZE", "1G"),
...
def delta_file(path: Path, indexes: tuple[int, ...]) -> float:
    if not path.exists():
        return 0.0
    rows = [line.split("\t") for line in path.read_text(encoding="utf-8").splitlines() if line.strip()]
    if len(rows) < 2:
        return 0.0
    return sum(float(rows[-1][idx]) - float(rows[0][idx]) for idx in indexes)
...
def run_once(config: Config, *, cycle: int, attempt: int, order: int,
             workload: str, block_size: int) -> dict[str, object]:
    ...
    if rc:
        return {
            **{field: 0 for field in RUN_FIELDS},
            "cycle": cycle, "attempt": attempt, "order": order, "workload": workload,
            "storage_block_size": block_size, "run_quality": "infra_error",
    ...
    return {
        "cycle": cycle, "attempt": attempt, "order": order, "workload": workload,
        "storage_block_size": block_size, "fio_block_size": summary["fio_block_size"],
        "write_mib_s": summary["write_mib_s"], "write_iops": summary["write_iops"],
        "write_lat_us": summary["write_lat_us"], "copy_calls": summary["copy_calls"],
        "copy_rows": summary["copy_rows"], "copy_exec_ms": summary["copy_exec_ms"],
        "copy_mean_ms": f"{copy_mean:.3f}", "copy_local_blks_written": summary["copy_local_blks_written"],
        "insert_calls": summary["insert_calls"], "insert_rows": summary["insert_rows"],
        "insert_exec_ms": summary["insert_exec_ms"], "insert_mean_ms": f"{insert_mean:.3f}",
        "insert_shared_blks_written": summary["insert_shared_blks_written"],
        "insert_wal_bytes": summary["insert_wal_bytes"], "wal_bytes_delta": f"{wal_bytes:.0f}",
        "wal_sync_time_delta_ms": f"{wal_sync:.3f}", "pg_io_wait_delta_ms": f"{io_wait:.3f}",
        ...
        "run_quality": "clean", "profile_artifact_dir": str(profile_dir),
    }
...
def main() -> int:
    ...
    cycle = 1
    while any(clean_counts[cell] < config.target_runs and attempts[cell] < config.max_attempts for cell in cells):
        ...
        for order, cell in enumerate(ordered, 1):
            ...
            workload, size = cell
            attempts[cell] += 1
            row = run_once(config, cycle=cycle, attempt=attempts[cell], order=order,
                           workload=workload, block_size=size)
            rows.append(row)
            if row["run_quality"] == "clean":
                clean_counts[cell] += 1
            write_tsv(runs_path, RUN_FIELDS, rows)
            print(f"progress workload={workload} block={size} clean={clean_counts[cell]}/{config.target_runs}")
        cycle += 1

    median_fields = [
        "workload", "storage_block_size", "attempts", "clean_runs", "infra_errors",
        "median_write_mib_s", "min_write_mib_s", "max_write_mib_s", "write_spread_pct",
        "median_write_iops", "median_write_lat_us", "median_copy_mean_ms", "median_insert_mean_ms",
        "median_wal_amplification", "median_copy_local_page_write_amplification",
        "median_insert_shared_page_write_amplification", "median_wal_sync_time_delta_ms",
        "median_pg_io_wait_delta_ms", "status",
    ]
    medians: list[dict[str, object]] = []
    selection_status = "valid"
    for workload, size in cells:
        all_cell = [r for r in rows if r["workload"] == workload and int(r["storage_block_size"]) == size]
        clean = [r for r in all_cell if r["run_quality"] == "clean"]
        ...
        medians.append({
            "workload": workload, "storage_block_size": size, "attempts": len(all_cell),
            "clean_runs": len(clean), "infra_errors": len(all_cell)-len(clean),
            "median_write_mib_s": f"{med:.3f}", "min_write_mib_s": f"{lo:.3f}",
            "max_write_mib_s": f"{hi:.3f}", "write_spread_pct": f"{spread:.2f}",
    ...
    comparison_fields = ["workload", "write_32k_mib_s", "write_64k_mib_s", "write_64k_vs_32k_pct",
                         "wal_amp_32k", "wal_amp_64k", "status"]
    comparisons: list[dict[str, object]] = []
    ratios: list[float] = []
    four_k_pct = 0.0
    by_key = {(r["workload"], int(r["storage_block_size"])): r for r in medians}
    for workload in WORKLOADS:
        r32, r64 = by_key[(workload, 32768)], by_key[(workload, 65536)]
        w32, w64 = float(r32["median_write_mib_s"]), float(r64["median_write_mib_s"])
        ...
        comparisons.append({
            "workload": workload, "write_32k_mib_s": f"{w32:.3f}", "write_64k_mib_s": f"{w64:.3f}",
            "write_64k_vs_32k_pct": f"{pct:.2f}", "wal_amp_32k": r32["median_wal_amplification"],
            "wal_amp_64k": r64["median_wal_amplification"], "status": status,
        })
