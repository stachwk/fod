...
repo_root="$(git rev-parse --show-toplevel 2>/dev/null)" || fail "must be run inside the FOD git repository"
repo_root="$(realpath -m "${repo_root}")"
manifest="${repo_root}/Cargo.toml"
expected_target="${repo_root}/target"
target="${FOD_TARGET_CLEAN_DIR:-${expected_target}}"
min_size="${FOD_TARGET_CLEAN_MIN_SIZE_BYTES:-10737418240}"
min_age_days="${FOD_TARGET_CLEAN_MIN_AGE_DAYS:-14}"
confirm="${FOD_TARGET_CLEAN_CONFIRM:-}"
force_raw="${FOD_TARGET_CLEAN_FORCE:-0}"
cargo_bin="${RUST_CARGO:-cargo}"

[[ -f "${manifest}" ]] || fail "Cargo.toml not found at ${manifest}"

case "${min_size}" in
  ''|*[!0-9]*) fail "FOD_TARGET_CLEAN_MIN_SIZE_BYTES must be a non-negative integer" ;;
esac
case "${min_age_days}" in
  ''|*[!0-9]*) fail "FOD_TARGET_CLEAN_MIN_AGE_DAYS must be a non-negative integer" ;;
esac
case "${force_raw}" in
  1|true|yes|on) force=1 ;;
  0|false|no|off|'') force=0 ;;
  *) fail "FOD_TARGET_CLEAN_FORCE must be 0/1, false/true, no/yes, or off/on" ;;
esac

[[ ! -L "${expected_target}" ]] || fail "refusing cleanup because repository target is a symlink: ${expected_target}"

target_abs="$(realpath -m "${target}")"
expected_abs="$(realpath -m "${expected_target}")"
[[ "${target_abs}" == "${expected_abs}" ]] || fail "refusing cleanup outside repository ./target: ${target_abs}"
...
if [[ -e "${target_abs}" && ! -d "${target_abs}" ]]; then
  fail "target exists but is not a directory: ${target_abs}"
fi

if [[ -d "${target_abs}" ]]; then
  target_exists=1
  fs_type="$(stat -f -c %T "${target_abs}")"
  [[ "${fs_type}" != "tmpfs" ]] || fail "refusing disk-target cleanup on tmpfs: ${target_abs}"
  target_size="$(du -sb -- "${target_abs}" | awk '{print $1}')"
  newest_epoch="$(find "${target_abs}" -mindepth 1 -printf '%T@\n' 2>/dev/null | awk 'BEGIN { max = 0 } { if ($1 > max) max = $1 } END { printf "%.0f\n", max }')"
  now_epoch="$(date +%s)"
...
run_cargo_dry_run() {
  cargo_supports_dry_run || fail "${cargo_bin} clean does not expose --dry-run; refusing controlled cleanup"
  "${cargo_bin}" clean --manifest-path "${manifest}" --target-dir "${target_abs}" --dry-run
}
...
if [[ "${command_name}" == "plan" ]]; then
  if [[ "${eligible}" -eq 1 ]]; then
    ...
    printf '%s\n' 'next=make target-disk-clean FOD_TARGET_CLEAN_CONFIRM=clean-disk-target'
  else
    printf '%s\n' 'decision=not-eligible'
    printf '%s\n' 'override=FOD_TARGET_CLEAN_FORCE=1 still requires FOD_TARGET_CLEAN_CONFIRM=clean-disk-target'
  fi
  exit 0
fi

[[ "${target_exists}" -eq 1 ]] || {
  printf '%s\n' 'Nothing to clean: repository ./target does not exist.'
  exit 0
}

if [[ "${eligible}" -ne 1 && "${force}" -ne 1 ]]; then
  fail "target is not eligible: require both size >= ${min_size} bytes and inactivity >= ${min_age_days} days"
fi

[[ "${confirm}" == "clean-disk-target" ]] || fail "explicit confirmation required: FOD_TARGET_CLEAN_CONFIRM=clean-disk-target"

before_size="${target_size}"
printf 'Cleaning persistent Cargo target through Cargo: %s\n' "${target_abs}"
"${cargo_bin}" clean --manifest-path "${manifest}" --target-dir "${target_abs}"
